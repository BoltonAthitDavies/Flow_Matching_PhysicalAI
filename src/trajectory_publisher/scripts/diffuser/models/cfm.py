...
# NeuralODE import deferred — inference uses torchdiffeq.odeint (Euler), not node.trajectory
import torchdiffeq
...
class CFM(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        # NeuralODE (torchdyn) not used in inference — deferred to avoid heavy dependency
        self.node = None
    ...
    def p_sample_loop_original(self, shape, global_cond, cond, verbose=True, return_diffusion=False):
        ...
        _utils = _get_utils(); progress = _utils.Progress(self.n_timesteps) if verbose else _utils.Silent()
